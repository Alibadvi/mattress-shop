...
export default async function AdminProductsList() {
  const items = await prisma.product.findMany({ include: { category: true }, orderBy: { createdAt: "desc" } });
          ...
          <tbody>
            {items.map(p => (
              <tr key={p.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-2 border-b">{p.inStock ? "In stock" : "Out of stock"}</td>
            ...
            {items.length === 0 && (
              <tr>
                <td className="px-4 py-6 text-center text-gray-500" colSpan={5}>No products yet.</td>
              </tr>
            )}
          </tbody>
        </table>
